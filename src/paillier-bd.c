...
void crt_exponentiation_bd(BIGD result, const BIGD base, const BIGD exp_p,
		const BIGD exp_q, const BIGD pinvq, const BIGD p, const BIGD q) {
...
void decrypt2_paillier_bd(BIGD ptxt, const BIGD ctxt, const paillier_bd_sk *key) {
	...
	// if (bdCompare(ptxt, threshold_bd) > 0)
	// 	bdSubtract(ptxt, ptxt, key->n);
